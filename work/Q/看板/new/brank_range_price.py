...
def get_sku_data_multi_month(cats, months):
    ...
    month_data = {
        m: {
            ...
            "brand_total_gmv": defaultdict(int),
            "seller_total": defaultdict(int),
            "total_order": 0,   # ✅ 新增：当月所有商品销量累加
        }
        for m in months
    }
    ...
    for d in cursor:
        ...
        for m in months:
            ...
            if order:
                ...
                month_data[m]["total_order"] += order  # ✅ 累加
...
def build_category_report(cat_id):
    ...
    for m in months:
        z_order_m = month_data[m]["total_order"]
        report["top10_brand_sales"][time[TIMES_4]][time[m]] = {
            b: {
                ...
                "price":month_data[m]["brand_total_gmv"].get(b, 0)/month_data[m]["brand_total"].get(b, 0) if month_data[m]["brand_total"].get(b, 0) else 0,
...
if __name__ == "__main__":
    ...
    print(f"Total to process: {total_count}")
    ...
    with mp.Pool(process_count) as pool:
        ...
        for cat_id in cat_list:
            if stop_event.is_set():
                break
            results.append(pool.apply_async(worker, args=(cat_id,)))
