        ...
        for t in time_list:
            ...
            bulk.append(
                UpdateOne(
                    {"cat_id": cat_id},
                    {"$set": {
                        f"ad_click_rate2.{t}": cur_doc["ad_click_rate"],
                        f"avg_cpc_usd2.{t}": cur_doc["avg_cpc_usd"],
                        f"avg_ad_cost_ratio2.{t}": cur_doc["avg_ad_cost_ratio"],
                        f"avg_refund_rate2.{t}": cur_doc["avg_refund_rate"],
                        f"total_gmv_usd.{t}": cur_doc["total_gmv_usd"],
                        f"top50_new_30d_sales_ratio.{t}": cur_doc["top50_new_30d_sales_ratio"]*100,
                        f"top50_new_90d_sales_ratio.{t}": cur_doc["top50_new_90d_sales_ratio"]*100,
                        f"top50_new_180d_sales_ratio.{t}": cur_doc["top50_new_180d_sales_ratio"]*100,
                        f"top50_avg_order_price.{t}": cur_doc["top50_avg_order_price"],
                        f"top50_seller_count.{t}": cur_doc["top50_seller_count"],
                        f"top50_brand_count.{t}": cur_doc["top50_brand_count"],
                        f"full_stock_avg_price.{t}": cur_doc["full_stock_avg_price"],
                        f"local_seller_avg_price.{t}": cur_doc["local_seller_avg_price"],
                        f"crossborder_seller_avg_price.{t}": cur_doc["crossborder_seller_avg_price"],
                        f"seller_concentration.{t}": cur_doc["seller_concentration"],
                        f"brand_concentration.{t}": cur_doc["brand_concentration"],
                        f"new_30d_sales_ratio.{t}": cur_doc["new_30d_sales_ratio"],
                        f"new_90d_sales_ratio.{t}": cur_doc["new_90d_sales_ratio"],
                        f"new_180d_sales_ratio.{t}": cur_doc["new_180d_sales_ratio"],
                    }}
                )
            )
