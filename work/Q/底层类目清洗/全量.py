...
# cat_ids = ["MLM4620","MLM172387","MLM178702"]
# cat_ids = visual_plus.find_one({"cat_id":"MLM436380"},{"_id":0,"cat":1})["cat"]
cat_ids = [doc["cat_id"] for doc in visual_plus.find({}, {"cat_id":1})]
SUCCESS_LOG = "success_full_cat_month.txt"
...
def fetch_and_compute_metrics(t, collection, cat_id):
    ...
    stock_type = p["stock_type"][f"{t}"] if p["stock_type"][f"{t}"] else 0
    ...
    stock_type_avg_price_ci = stock_type_avg_price.get("ci",0)
    stock_type_avg_price_nor = stock_type_avg_price.get("nor",0)
    stock_type_avg_price_ful = stock_type_avg_price.get("ful",0)
    ...
    result_doc = {
        ...
        "total_avg_price": to_round(gmv.get(str(t), 0)/order.get(str(t)) if order.get(str(t)) else get_nested(doc, "avg_price")),  # 全量_成交平均价
